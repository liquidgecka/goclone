...
static void join_cgroups(goclone_cmd *cmd) {;
	...
	for (i = 0; cmd->cgroups_tasks_files[i] != NULL; i++) {
		...
		if (write(fd, buffer, len) != len) { _exit(EX_OSERR); }
...
static void join_namespace(char *namespace) {
	...
	if (fd == -1) { _exit(EX_OSERR); }
	if (setns(fd, 0) != 0) { _exit(EX_OSERR); }
	if (close(fd) != 0) { _exit(EX_OSERR); }
}
...
static void setup_directories(goclone_cmd *cmd) {
	...
	if (cmd->chroot_dir != NULL && cmd->chroot_dir[0] != '\0') {
		if (chroot(cmd->chroot_dir) == -1) { _exit(EX_OSERR); }
	}

	// Chdir
	if (cmd->dir != NULL && cmd->dir[0] != '\0') {
		if (chdir(cmd->dir) == -1) { _exit(EX_OSERR); }
	} else {
		if (chdir("/") == -1) { _exit(EX_OSERR); }
	}
}
...
static void set_credentials(goclone_cmd *cmd) {
	...
	if (setregid(cmd->gid, cmd->gid) != 0) { _exit(EX_OSERR); }
	if (getgid() != cmd->gid) { _exit(EX_OSERR); }
	...
	if (setreuid(cmd->uid, cmd->uid) != 0) { _exit(EX_OSERR); }
	if (getuid() != cmd->uid) { _exit(EX_OSERR); }
}
...
static int clone_flags(goclone_cmd *cmd)
{
	...
	if (cmd->new_ipc_namespace) { flags |= CLONE_NEWIPC; }
	if (cmd->new_mount_namespace) { flags |= CLONE_NEWNS; }
	if (cmd->new_network_namespace) { flags |= CLONE_NEWNET; }
	if (cmd->new_pid_namespace) { flags |= CLONE_NEWPID; }
	if (cmd->new_user_namespace) { flags |= CLONE_NEWUSER; }
	if (cmd->new_uts_namespace) { flags |= CLONE_NEWUTS; }
	return flags;
}
